...
def parse_pbf(filename, bbox):
    ...
    if natural is not None:
        ...
        polygons = concat([polygons, natural.loc[natural.geometry.type == 'Polygon']])
        multipolygons = concat([multipolygons, natural.loc[natural.geometry.type == 'MultiPolygon']])

    landuse = osm.get_landuse()
    if landuse is not None:
        landuse = landuse.loc[:, ['landuse', 'geometry']].rename(columns={'landuse': 'tag'})
        polygons = concat([polygons, landuse.loc[landuse.geometry.type == 'Polygon']])
        multipolygons = concat([multipolygons, landuse.loc[landuse.geometry.type == 'MultiPolygon']])

    for i in range(multipolygons.shape[0]):
        tag = multipolygons.tag.iloc[i]
        for polygon in multipolygons.geometry.iloc[i].geoms:
            polygons = concat([polygons, DataFrame({'tag': tag, 'geometry': polygon}, index=[0])], ignore_index=True)
    ...
    if edges is not None:
        ...
        edges = edges[["highway", "geometry", "u", "v", "length"]].rename(columns={"highway": "tag"})
        ...
        if child.tag == 'node':
            ...
            for part in child:
                if part.tag == 'nd':
                    ...
                    tag = part.attrib['v']
            if way[0] == way[-1]:
                geometry = Polygon([nodes[pt] for pt in way if pt in nodes.keys()])
                if tag is None:
                    ways[child.attrib['id']] = geometry
                else:
                    polygons.append((tag, geometry))
            else:
                geometry = LineString([nodes[pt] for pt in way if pt in nodes.keys()])
                if tag is None:
                    ways[child.attrib['id']] = geometry
                else:
            ...
            if tag is not None:
                if len(relation[0]) == 1:
                    ...
                    outer = MultiLineString([ways[way] for way in relation[0] if way in ways.keys()]).convex_hull
                inner = [ways[way] for way in relation[1] if way in ways.keys() and isinstance(ways[way], Polygon)]
                geometry = Polygon(outer, inner)
                if not geometry.is_empty:
                    polygons.append((tag, geometry))

    polygons = GeoDataFrame(polygons, columns=['tag', 'geometry'], crs='epsg:4326')
    roads = DataFrame(roads, columns=['tag', 'geometry'])
    roads.geometry = roads.geometry.apply(lambda x: [[x[i], x[i + 1]] for i in range(len(x) - 1)])
    roads = roads.explode('geometry').reset_index(drop=True)
    roads[['u', 'v']] = DataFrame(roads.geometry.tolist(), index=roads.index)
    roads.geometry = roads.apply(lambda x: LineString((nodes[x.u], nodes[x.v])), axis=1)
    roads['length'] = roads.apply(lambda x: point_distance(nodes[x.u], nodes[x.v]), axis=1) * 1000
    roads = GeoDataFrame(roads, crs='epsg:4326')
    nodes = {k: v for k, v in nodes.items() if k in (set(roads.u) | set(roads.v))}
