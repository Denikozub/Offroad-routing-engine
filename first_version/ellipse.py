...
class VisibilityGraph:
    ...
    def __init__(self, filename, bbox):
        ...
        self.initial_multilinestrings = pd.DataFrame(natural.loc[:, ['natural', 'geometry']].loc[natural.geometry.type == 'MultiLineString']).head(0)
    ...
    def incident_vertices(self, point_data, pair_func, add_edges_inside=True, inside_percent=1):
        ...
        for i in range(polygon_count):
            ...
            if point_polygon_number is not None and i == point_polygon_number:
                if add_edges_inside:
                    edges_inside = self.__add_inside_poly(point, point_polygon_point_number, polygon.geometry, i, inside_percent)
            ...
            # elif not Polygon(polygon.convex_hull).contains(Point(point)):
                ...
                if line is None:
                    return self.__add_inside_poly(point, None, polygon.geometry, i, 1)
    ...
    def __process_points_of_objects(self, is_polygon, G, plot, pair_func, add_edges_inside, inside_percent):
        ...
        object_count = self.polygons.shape[0] if is_polygon == 'polygon' else self.multilinestrings.shape[0]
        for i in range(object_count):
            obj = self.polygons.geometry[i] if is_polygon == 'polygon' else self.multilinestrings.geometry[i]
            point_count = len(obj) - 1 if is_polygon == 'polygon' else len(obj)
            for j in range(point_count):
                point = obj[j]
                px, py = point
                point_data = {'point' : point, 'obj_number' : i, 'point_number' : j, 'is_polygon' : is_polygon)
                ...
                for vertex in vertices:
                    vx, vy = vertex['point']         # x and y coordinates of vertex
                    vertex_index = vertex['obj_number'] * max_poly_len + vertex['point_number'] if vertex['is_polygon'] \
                            else (vertex['obj_number'] + 0.5) * max_poly_len + vertex['point_number']
                    G.add_node(vertex_index, x=vx, y=vy)
                    G.add_edge(point_index, vertex_index)
                    if plot is not None:
                        plt.plot([px, vx], [py, vy], color=plot[1])
